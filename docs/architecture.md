...
- `bot/bot/services` — Business logic: key lifecycle, billing, user flows, admin/query services, server control.
...
- `bot/worker_main.py` — standalone background worker for NATS consumer processing.
...
2. A handler in `bot/bot/handlers` parses the update and delegates to a function in `bot/bot/services`.
...
- NATS consumers: job consumers subscribe to JetStream streams to process tasks such as key removal or other long-running jobs.
- The active remove-key consumer runs in the standalone worker entrypoint `bot/worker_main.py`, not inside the polling bot runtime.
...
Deployment notes
- Docker Compose is the primary deployment descriptor: `docker-compose.yml` wires up `postgres`, `nats`, `vpn_hub_bot`, `nats-worker` and other services.
